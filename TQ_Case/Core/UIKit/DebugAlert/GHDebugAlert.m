...
#import "GHDebugAlert.h"
#import <Masonry/Masonry.h>

@interface GHDebugAlert ()
...
static NSMutableArray<GHDebugAlert *> *alertArray = nil;
...
- (void)dismissAlert {
    ...
    [self _dismiss:^(BOOL finished) {
        ...
        GHDebugAlert *alert = alertArray.firstObject;
        if (alert) {
            [alert _show:nil];
        }
    }];
...
- (DebugAlertFormatBlock)message {
    return ^GHDebugAlert *(NSString *format, ...) {
        NSString *text = nil;
        if (format) {
            va_list args;
...
- (DebugAlertStyleBlock)style {
    return ^GHDebugAlert *(DebugAlertStyle style) {
...
- (DebugAlertBoolBlock)autoDismiss {
    return ^GHDebugAlert *(BOOL autoDismiss) {
