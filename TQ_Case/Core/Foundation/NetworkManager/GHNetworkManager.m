...
DDLogInfo(@"响应JSON：%@", [responseObject JSONString]);\
...
- (nullable NSURLSessionDataTask *)HTTP_GET:(NSString *)url action:(nullable NSString *)action
    ...
    //请求参数
    NSDictionary *retParams = [GHNetworkUtil requestParamsWithApi:action params:tempDict];
...
- (nullable NSURLSessionDataTask *)HTTP_POST:(NSString *)url action:(nullable NSString *)action
    ...
    //请求参数
    NSString *retUrl = [GHNetworkUtil requestUrlWithUrl:url api:action];
    NSDictionary *retParams = [GHNetworkUtil requestParamsWithApi:action params:tempDict];
