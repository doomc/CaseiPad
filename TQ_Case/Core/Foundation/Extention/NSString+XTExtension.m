...
- (NSString *)cutJson{
    ...
    if(self.length > 0){
        ...
        NSRegularExpression *regularExpretion=[NSRegularExpression regularExpressionWithPattern:@"json=\\{.*,'content':.+\\}"
                                                                                        options:0
                                                                                          error:nil];
        for(NSString *url in urlArray){
            NSArray* match = [regularExpretion matchesInString:url options:NSMatchingReportCompletion range:NSMakeRange(0, [url length])];
            ...
            if (match.count != 0)
            {
                for (NSTextCheckingResult *matc in match)
                {
                    ...
                    if(finalUrl == nil){
                        finalUrl = [[[url substringWithRange:range] stringByReplacingOccurrencesOfString:@"json=" withString:@""] stringByReplacingOccurrencesOfString:@"'" withString:@"\""];
                    }else{
                        finalUrl = [finalUrl stringByAppendingFormat:@"|%@",[url substringFromIndex:range.length]];
                    }
...
- (BOOL)isIdCard18{
    NSPredicate *	pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", Regex_18IDCard];
    return [pred evaluateWithObject:self];
}
...
- (BOOL)isSuffix:(NSString *)suffix{
    NSPredicate *	pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", suffix];
    return [pred evaluateWithObject:self];
}
