...
// 用户密码
#define Regex_Password      @"^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$"
...
#define Regex_UrlJson       @"json=\{.*,'content':.+\}"
...
- (CGSize)sizeWithFont:(UIFont *)font byWidth:(CGFloat)height textLineSpacing:(CGFloat)textHeight ;
- (CGSize)sizeWithFont:(UIFont *)font byHeight:(CGFloat)height textLineSpacing:(CGFloat)textHeight;

@end

@interface NSString (XTAttribute)
